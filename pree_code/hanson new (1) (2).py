...
# Download proxy for U.S. interest rate (13-week T-bill rate)
interest_rate_proxy = yf.download('^IRX', start=start_date, end=end_date)['Close'].squeeze()

# Download crude oil price (WTI)
oil_price = yf.download('CL=F', start=start_date, end=end_date)['Close'].squeeze()

# Placeholder for manual macroeconomic controls
#Replace these with actual CSV imports or API calls
# Manual macroeconomic controls (simulated data placeholders for now)
# Inflation (YoY %)
...
gdp_data = pd.read_csv('C:/Users/prem_/Downloads/RUS GDP.csv')
# Convert 'Year' to datetime for alignment
gdp_data['Year'] = pd.to_datetime(gdp_data['Year'], format='%Y')
# Create a datetime index spanning the year
gdp_data.set_index('Year', inplace=True)
# Reindex to daily frequency and forward-fill the annual values
gdp_daily = gdp_data.resample('D').ffill()
# Align with the exchange rate index
gdp_growth = gdp_daily.reindex(usdt_cad.index, method='ffill')['Value']
print("GDP values")
print (gdp_growth)
...
##NEW
#Add controls:Technical trend USDT/CAD price, BTC Price, Interest Rates (US), Oil Price (Global or Russia), Inflation (Russia simulated), GDP Growth (Russia simulated), FX_Reserves (Russia simulated), Trade Balance (Russia simulated)
# create a constant/intercept column
iv_data = data.copy()
iv_data = sm.add_constant(iv_data)

# Run IV regression
iv_model = IV2SLS(
    dependent=iv_data['RUB_exchange_rate'],
    exog=iv_data[['const', 'USDT_CAD_MA', 'BTC_Price', 'Interest_Rate', 'Oil_Price',
...
X_dwh = sm.add_constant(data[['RU_stablecoin_price', 'USDT_CAD_MA', 'BTC_Price', 'Interest_Rate',
                              'Oil_Price', 'Inflation', 'GDP_Growth', 'FX_Reserves',
                              'Trade_Balance', 'first_stage_resid']])
